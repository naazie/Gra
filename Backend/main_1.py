...
from bactracker import compute_gaps_for_topic, detect_intent
...
async def chat(
    ...
    try:
        ...
        next_gap     = None
        intent_flags = detect_intent(display_text, entities)

        if intent_flags.get("is_delivery_request"):
            from session_state import clear_gaps
            clear_gaps(session_id)
            active_gap = None

        if is_query and intent_flags["is_study_request"] and entities:
            topic = intent_flags["topic"]

            # 1. Try curriculum graph first (reliable)
            curriculum_prereqs = get_curriculum_prerequisites(graph, topic)
            curr_topics        = [p["topic"] for p in curriculum_prereqs]

            # 2. Supplement with LLM if curriculum graph has nothing
            if not curr_topics:
                try:
                    curr_topics = await fetch_requirements(f"study {topic}")
                except Exception:
                    curr_topics = []

            if curr_topics:
                gaps = compute_gaps_for_topic(
                    graph, username, topic,
                    llm_requirements=curr_topics,
                    embedder=_get_embedder(),
                    faiss_mgr=_get_faiss(),
                )
                if gaps:
                    set_gap_queue(session_id, gaps, topic, goal=display_text)
                    next_gap = active_gap = gaps[0]

        elif active_gap and intent_flags["is_gap_answer"]:
            if background_tasks:
                background_tasks.add_task(pipeline.ingest, user_id=username, raw_text=display_text)
            next_gap   = advance_gap(session_id)
            active_gap = next_gap

        elif should_ask_next_gap(session_id):
            next_gap = get_active_gap(session_id)
        ...
        review_note   = ""
        if review_topics and not active_gap and not next_gap:
        ...
        goal_context = ""
        if intent_flags.get("has_goal") and entities:
            goal_context = f"User's stated goal this session: {display_text[:100]}"
        ...
        if not stream:
            ...
            answer = await generate_answer_async(
                ...
                backtrack_question=active_gap if (next_gap and active_gap) else "",
            ...
            return {
                ...
                "corrections": corrections,
                "session_id": session_id,
                "backtrack_active": bool(next_gap),   # only True when LLM was asked a prereq question this turn
        ...
        if active_gap and next_gap:
            parts.append(f"\nNaturally ask how well they know: {active_gap}")
...
async def get_weak_areas(username: str):
    ...
    try:
        ...
        with graph.driver.session() as session:
            ...
            """, un=username)
            for r in r1:
                raw_conf = r["conf"] or 1.0
                effective_conf = raw_conf if raw_conf < 0.95 else 0.35
                areas.append({
                    "topic": r["topic"], "type": "weak_in",
                    "confidence": round(effective_conf * 100),
...
#  Fixes: all async, all authenticated, no duplicate route conflicts
# ══════════════════════════════════════════════════════════════════════════════

@app.get("/memory/mindmap")
async def get_mindmap(username: str = Depends(_verify_token)):
    """§1.8 spec — returns nodes[], edges[], metadata."""
    try:
        ...
        return {
            ...
            "metadata": {
                ...
                "memory_count":  len([n for n in data["nodes"] if n["type"] == "Memory"]),
                "concept_count": len([n for n in data["nodes"] if n["type"] == "Concept"]),
            },
        }
    except Exception as e: raise HTTPException(500, str(e))
...
async def get_graph_visuals(username: str = Depends(_verify_token)):
    """Returns {nodes, links} for the 3D mindmap. Auth required."""
    try:
...
async def recall_memories(username: str = Depends(_verify_token)):
    """Called by Chat.jsx after every message to refresh the graph."""
    try:
...
async def get_stats(username: str = Depends(_verify_token)):
    """Topbar stats — memory count, concept count, edge count."""
    try:
        visual_data   = graph.get_visual_graph(username)
        memory_count  = len([n for n in visual_data["nodes"] if n["type"] == "Memory"])
