# 🧠 GraphMind v11.0 — Hybrid RAG Memory Engine
...
## What's new in v11.0
- **Fix**: Single Neo4j GraphEngine — orchestrator now receives the shared instance from `main.py` via `init_orchestrator()`, eliminating the duplicate bolt connection
- **Fix**: Chat and Sidebar background stars replaced with pure CSS particle animation (`CssStars.jsx`) — removes 2 of 3 WebGL contexts, safe on any hardware
- **Fix**: `color-mix()` replaced with static `rgba()` values in Sidebar — full cross-browser compatibility
- **Improvement**: Contradiction check now runs on every query with entities, not only when Llama's `potential_conflict` flag fires
- **Backend**: New `GET /stats/{username}` endpoint for live topbar numbers

## What's new in v5.0
- **Fix**: `Query` imported from neo4j — concept-to-concept edges now store correctly
- **Performance**: `Embedder` and `FAISSManager` are now lazy singletons — sub-100ms retrieval actually achieved after warmup (was 800-2000ms per query due to model reload)
- **Fix**: Streaming endpoint uses `OLLAMA_BASE_URL` env var consistently
- **Demo**: `seed_demo.py` pre-loads 12 rich interview-prep memories for live presentations
...
### 4. Seed demo data (recommended for presentation)
```bash
cd Backend
python seed_demo.py
# Loads 12 rich memories for user 'demo' / 'graphmind2026'
# Run once before your demo — takes ~2 min (Gemini rate limits)
```
