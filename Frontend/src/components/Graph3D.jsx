  ...
  return (
    <div style={{ flex: 1, height: "100%", position: "relative" }}>
      ...
      >
        ...
        {/* Nodes */}
        {nodes.map((node, idx) => {
          const nodeIsCited = isCited(node);
          // citedIndex: position in the cited list — drives the 300ms stagger
          const citedIndex = nodeIsCited
            ? [...citedSet].indexOf(String(node.id)) !== -1
              ? [...citedSet].indexOf(String(node.id))
              : [...citedSet].indexOf(String(node?.metadata?.uuid)) 
            : 0;
          return (
            <SkillNode
              key={node.id}
              ...
              isCited={nodeIsCited}
              citedIndex={Math.max(0, citedIndex)}
            />
          );
        })}
